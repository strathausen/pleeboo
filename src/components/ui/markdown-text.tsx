...
export function MarkdownText({ text, className }: MarkdownTextProps) {
  const html = useMemo(() => {
    ...
    processed = processed.replace(/^### (.+)$/gm, '<h3 class="font-semibold text-sm mt-2 mb-1">$1</h3>');
    processed = processed.replace(/^## (.+)$/gm, '<h2 class="font-semibold text-base mt-3 mb-1">$1</h2>');
    processed = processed.replace(/^# (.+)$/gm, '<h1 class="font-bold text-lg mt-3 mb-2">$1</h1>');
